...
var (
	...
	output      = flag.String("output", "", "output file name; default srcdir/<type>_enum2string.go")
	pkgName     = flag.String("pkg", "", "output package name")
	samepkg     = flag.Bool("samepkg", false, "enum2string and string2enum output in same package")
	enumpkg     = flag.Bool("enumpkg", false, "enum2string and string2enum output in the enum package")
...
func Usage() {
	fmt.Fprintf(os.Stderr, "Usage of enum2string:\n")
	fmt.Fprintf(os.Stderr, "\tenum2string [flags] -type T [directory]\n")
	fmt.Fprintf(os.Stderr, "\tenum2string [flags] -type T files... # Must be a single package\n")
...
func main() {
	...
	g.Printf("// Code generated by \"enum2string %s\"; DO NOT EDIT.\n", strings.Join(os.Args[1:], " "))
	g.Printf("\n")
	...
	outputName := *output
	if outputName == "" {
		baseName := fmt.Sprintf("%s_enum2string.go", types[0])
...
func (g *Generator) generate(typeName string) {
	...
	if !*samepkg {
		...
		g.Printf("\t// Re-run the enum2string command to generate them again.\n")
...
func (f *File) genDecl(node ast.Node) bool {
	...
	for _, spec := range decl.Specs {
		...
		if vspec.Type != nil {
			...
			switch vspecType := vspec.Type.(type) {
			case *ast.Ident:
				typ = vspecType.Name
			case *ast.SelectorExpr:
				typ = vspecType.Sel.Name
			default:
				continue
			}
		}
...
func (g *Generator) buildOneRun(runs [][]Value, typeName string) {
	...
	if values[0].value == 0 { // Signed or unsigned, 0 is still 0.
		g.Printf(stringOneRun, typeName, usize(len(values)), lessThanZero, typeIdent)
	} else {
		g.Printf(stringOneRunWithOffset, typeName, values[0].String(), usize(len(values)), lessThanZero, typeIdent)
	}
}
...
func %[1]sString(v %[4]s) (string, bool) {
	...
	return _%[1]s_name[_%[1]s_index[i]:_%[1]s_index[i+1]], true
}
`
...
`
	...
	for i, values := range runs {
		if len(values) == 1 {
			g.Printf("\tcase i == %s:\n", &values[0])
			g.Printf("\t\treturn _%s_name_%d, true\n", typeName, i)
			continue
		}
		...
		g.Printf("\t\treturn _%s_name_%d[_%s_index_%d[i]:_%s_index_%d[i+1]], true\n",
